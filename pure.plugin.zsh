...
prompt_pure_preprompt_render() {
	...
	# in bash (https://superuser.com/questions/974908/multiline-rprompt-in-zsh)
	# cant make it work
	# $(( ${#${(S%%)str//(\%([KF1]|)\{*\}|\%[Bbkf])}} ))

	# this is the first line of the prompt
	# and only the left side part
	# Join preprompt parts, space separated.
	prompt_left=${(j. .)preprompt_parts}
	# expand it to the actual text
	prompt_left_expand="${(S%%)prompt_left}"
	# remove all color and other invisible ansi characters
	prompt_left_expand_raw="$(echo $prompt_left_expand | sed 's/\x1B[@A-Z\\\]^_]\|\x1B\[[0-9:;<=>?]*[-!"#$%&'"'"'()*+,.\/]*[][\\@A-Z^_`a-z{|}~]//g')"
	# calc its lenght
	prompt_left_length=${#prompt_left_expand_raw}

	# install https://github.com/jonmosco/kube-ps1
	# on the right side we print the kube_ps1 output
	prompt_right=$(kube_ps1)
	# expand that
	prompt_right_expand="${(S%%)prompt_right}"
	# remove all color and other invisible ansi characters
	prompt_right_expand_raw="$(echo $prompt_right_expand | sed 's/\x1B[@A-Z\\\]^_]\|\x1B\[[0-9:;<=>?]*[-!"#$%&'"'"'()*+,.\/]*[][\\@A-Z^_`a-z{|}~]//g')"
	# calc its lenght
	prompt_right_length=${#prompt_right_expand_raw}


	filler_length=$(( COLUMNS - prompt_right_length - prompt_left_length))
	filler=${(l:$filler_length:)}

	ps1=(
		$prompt_left$filler$prompt_right 		# Join left and right side
		$prompt_newline           					# print \n character
		$cleaned_ps1												# print line to type in
	)
